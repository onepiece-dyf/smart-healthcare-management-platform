  ...
  actions: {
    async fetchOrdersPage(
      ...
      try {
        ...
        // 构建数据库查询
        let query = db.orders.orderBy('createdAt');
        
        // 应用筛选条件
        if (filters?.type) {
          query = query.filter(order => order.type === filters.type);
        }
        
        if (filters?.status?.length) {
          query = query.filter(order => filters.status!.includes(order.status));
        }
        
        if (filters?.priority?.length) {
          query = query.filter(order => order.priority && filters.priority!.includes(order.priority));
        }
        
        if (filters?.doctorDepartment?.length) {
          query = query.filter(order => order.doctorDepartment && filters.doctorDepartment!.includes(order.doctorDepartment));
        }
        
        if (filters?.searchText) {
          const searchText = filters.searchText.toLowerCase();
          query = query.filter(order => 
            order.content.toLowerCase().includes(searchText) ||
        ...
        const orders = await query.offset(offset).limit(pageSize).toArray();
        ...
        this.error = error instanceof Error ? error.message : '获取医嘱数据失败';
    ...
    async fetchOrderById(id: number) {
      try {
        ...
        if (order) {
          order.patient = await usePatientStore().getPatientById(
            order.patientId
          );
          order.doctor = await db.users.get(order.doctorId);
        }
